...
class MY_Model extends CI_Model
{
    ...
    public function send_email($subject, $email_content, $user_id, $check_replace = true)
    {
        ...
        try {
            ...
            $email_protocol_type = trim(get_option("email_protocol_type", ""));
            $smtp_server = trim(get_option("smtp_server", ""));
            ...
            $smtp_encryption = trim(get_option("smtp_encryption", ""));
            if ($smtp_encryption === 'none') {
                $smtp_encryption = '';
            }

            if ($email_protocol_type == "smtp") {
                if ($smtp_server == "" || $smtp_port == "" || $smtp_username == "" || $smtp_password == "") {
                    return "SMTP is selected but SMTP settings are incomplete.";
                }
