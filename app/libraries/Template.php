	...
	private function _find_view($view, array $data, $parse_view = TRUE)
	{
		...
		if ( ! empty($this->_theme))
		{
			foreach ($this->_theme_locations as $location)
			{
				...
				// Module-specific themed override
				if ( ! empty($this->_module))
				{
					$theme_views[] = $this->_theme . '/views/modules/' . $this->_module . '/' . $view;
				}

				// Only allow generic theme overrides for explicit paths or non-module requests.
				// This prevents generic files like themes/<theme>/views/index.php from hijacking
				// module views such as services/index, client/index, api/index, etc.
				$normalized_view = str_replace('\\', '/', $view);
				if (empty($this->_module) || strpos($normalized_view, '/') !== FALSE)
				{
					$theme_views[] = $this->_theme . '/views/' . $view;
				}
