...
<script>
  $(document).ready(function () {
    var raw_services = <?php echo json_encode($items_service ?? [], JSON_UNESCAPED_UNICODE); ?>;
    var currency_symbol = '<?php echo get_option('currency_symbol'); ?>';
    
    var selectizeOptions = raw_services.map(function(service) {
      var data_service = {
        service_id: service.id,
        rate: parseFloat(service.price),
        original_rate: parseFloat(service.original_price),
        name: service.name
      };
      return {
        value: JSON.stringify(data_service),
        text: service.id + " - " + service.name + " [" + currency_symbol + parseFloat(service.price) + "]"
      };
    });
