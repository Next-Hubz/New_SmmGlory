...
var alertMessage = alertMessage || (function ($) {
  var $html = $('<div class="alert alert-icon content d-none" role="alert">' +
    ...
    '</div>');
  return {
    show: function (_message, _type, _context) {
      switch (_type) {
        case 'error':
      ...
      var $target = _context ? $(_context).find('.alert-message-reponse') : $('.alert-message-reponse');
      if ($target.length === 0 && _context) {
        $target = $('.alert-message-reponse');
      }

      $target.html($html.clone());
      var $content = $target.find('.content');
      var $iconSymbol = $target.find('.icon-symbol');

      if (_type == 'alert-success') {
        $content.removeClass('alert-warning');
      } else {
        $content.removeClass('alert-success');
      }
      $content.addClass(_type);
      $iconSymbol.addClass(_icon);
      $content.removeClass('d-none');
      $content.find('.message').html(_message);
    },

    hide: function (_context) {
      var $target = _context ? $(_context).find('.alert-message-reponse') : $('.alert-message-reponse');
      if ($target.length === 0 && _context) {
        $target = $('.alert-message-reponse');
      }
