...
function General(){
    ...
    this.General = function() {
        ...
        $(document).on("submit", ".actionForm", function () {
            ...
            toggleSubmitButtonState(_that, true);
            ...
            $.post(_action, _data, function (_result) {
                setTimeout(function () {
                    toggleSubmitButtonState(_that, false);
                    pageOverlay.hide();
                }, 1500)
        ...
        $(document).on("submit", ".actionFormWithoutToast", function(){
            ...
            $.post(_action, _data, function (_result) {
                if (is_json(_result)) {
                    _result = JSON.parse(_result);
                    setTimeout(function () {
                        alertMessage.show(_result.message, _result.status, _that);
                    }, 1500)
...
function toggleSubmitButtonState($form, isLoading) {
    var $buttons = $form.find('button[type="submit"], .btn-submit, .btn-spinner-border');
    $buttons.each(function () {
        var $button = $(this);
        if (isLoading) {
            $button.addClass('loading btn-loading').prop('disabled', true);
        } else {
            $button.removeClass('loading btn-loading').prop('disabled', false);
        }
    });
}
